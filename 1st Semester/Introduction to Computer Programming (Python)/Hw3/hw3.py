...
def nearest_location(loc, loc_list, loc_type=None):
    ...
    """GRAPSTE TON KWDIKA SAS APO KATW."""
    if loc_type==None:
        list_LocDist=[[x, distance(loc,x)] for x in loc_list if x!=loc]
    else:
        list_LocDist=[[x, distance(loc, x)] for x in loc_list if type(x)==loc_type and x!=loc]
        
    minimum = lambda x,y: x if x[1]<y[1] else y 
    from functools import reduce
    return reduce(minimum, list_LocDist)[0]
...
def flatten(field):
    ...
    help_field=field
    flatt_ls=[field[0]]
    while help_field[1]!=None:
        help_field=help_field[1]
        flatt_ls=flatt_ls + [help_field[0]]
    return flatt_ls
