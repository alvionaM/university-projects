...
def paei_pi(n):
    ...
    """
    def oros(n):
        return 4/(2*n-1)*(-1)**(n-1)#me to (-1)**(n-1) prokyptei to proshmo toy orou synarthsei toy deikth n 
    return sum_sequence(n, oros)
...
def askisi9():
    ...
    perifereia=lambda r: 2*pi*r
    tetragwno=lambda x: x*x
    emvado=lambda r: pi*r*r
    def repeat(f,n):
        for i in range(1,n+1):
            print (f(i)) if n!=5 else print("Emvado kyklou aktinas", i, "=", f(i))
    print("Perifereies kyklwn me aktina 1 ews 10")
    repeat(perifereia, 10)
