...
def print_board(board):
    ...
    for cell in get_iterator(board):
        ...
        if cell[0][1]== n: #eftase sto telos ths grammhs 
            print()
...
def tick(board):
    ...
    for item in get_iterator(copy):
        cell=item[0]
        i=0 #metrhths zwntanwn geitonikwn keliwn 
        ...
        if is_alive(copy,cell):
             ...
             #an i==2 'h i==3 tote to cell paramenei zwntano (dhl. akrivws opws htan)
             elif i>=4:
                set_alive(board, cell, False)
        else:
             if i==3:
...
if __name__ == '__main__':
    ...
    for i in range(100): 
        print_board(game) #emfanizetai kai o pinakas ths arxikhs topo8ethshs 
        tick(game)
        sleep(2)
        print()
        print()
